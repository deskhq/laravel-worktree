...
/**
 * A worked example: the-desk's real config/worktree.php.
 *
 * The shipped config/worktree.php documents every key and every trap, but it
 * configures nothing — a three-step toy teaches less than one application's
 * actual recipe, and this is the one that produced most of the warnings in
 * those comments. It replaced a thousand lines of bash, and the issue numbers
 * in the comments below are that application's.
 *
 * Copy the parts that apply; none of it is defaults, and a repository that
 * ships no config file at all still runs.
 *
 * Not published by `vendor:publish` — it is documentation, and publishing it
 * over an application's own config would be the opposite of helpful. A test
 * loads it through the same validator the binary uses, so an example that
 * stopped being legal fails this package's suite rather than somebody's create.
 */
return [
